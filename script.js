...
// เกณฑ์กำลังไฟเพื่อเริ่มรอบ (วัตต์) สามารถตั้งค่าได้ใน UI
let workingThreshold = parseFloat(localStorage.getItem('workingThreshold')) || 5;
...
function updateUI({ power, energy, cycles, switchState, online, error }) {
  ...
  if (workingStatusEl) {
    if (error) {
      ...
      workingStatusEl.textContent = isWorking ? 'กำลังทำงาน' : 'ไม่ทำงาน';
    }
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // ตั้งค่าอินพุตเกณฑ์กำลังไฟเริ่มรอบ
  const thresholdInput = document.getElementById('thresholdInput');
  if (thresholdInput) {
    thresholdInput.value = workingThreshold.toFixed(2);
    thresholdInput.addEventListener('change', () => {
      const val = parseFloat(thresholdInput.value);
      if (!isNaN(val) && val >= 0) {
        workingThreshold = val;
        localStorage.setItem('workingThreshold', workingThreshold);
      }
    });
  }
